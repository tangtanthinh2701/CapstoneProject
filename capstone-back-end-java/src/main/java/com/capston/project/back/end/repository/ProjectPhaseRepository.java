...
public interface ProjectPhaseRepository extends JpaRepository<ProjectPhase, Integer> {
	...
	List<ProjectPhase> findByProject_IdOrderByPhaseNumberAsc(Integer projectId);

	Optional<ProjectPhase> findByProject_IdAndPhaseNumber(Integer projectId, Integer phaseNumber);

	boolean existsByProject_IdAndPhaseNumber(Integer projectId, Integer phaseNumber);

	// Đếm số phase theo status của project
	long countByProject_IdAndPhaseStatus(Integer projectId, PhaseStatus status);

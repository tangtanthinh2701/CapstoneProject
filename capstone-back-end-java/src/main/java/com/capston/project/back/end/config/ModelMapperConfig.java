	...
	public ModelMapper modelMapper() {
		...
		modelMapper.addMappings(new PropertyMap<ProjectRequest, Project>() {
			...
			protected void configure() {
				...
				skip(destination.getBudget());
				skip(destination.getTargetConsumedCarbon());
				skip(destination. getCurrentConsumedCarbon());
		...
		// Custom mapping cho ProjectPhaseRequest -> ProjectPhase (bỏ qua computed fields)
		...
		Converter<PhaseStatus, PhaseStatus> phaseStatusConverter = ctx ->
				ctx.getSource() != null ? ctx.getSource() : PhaseStatus.PLANNING;
